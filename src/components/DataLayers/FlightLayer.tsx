...
export default function FlightLayer({
  ...
  const activeFlights = enabled
    ? showMilitary
      ? flights
      : flights.filter((f) => !f.isMilitary)
    : showMilitary
      ? flights.filter((f) => f.isMilitary)
      : [];

  const updateBillboards = useCallback(async () => {
    ...
    import("cesium").then((Cesium) => {
      ...
      handler.setInputAction((click: any) => {
        ...
        if (
          ...
          const flight = flightMapRef.current.get(picked.id);
